...
prependList <- function(x, new_val){
  out <- c(new_val, x)
  return(out)
}
...
predictFirst <- function(l_diffs, curr_lvl=length(l_diffs)){
  ...
  if(curr_lvl==0){
    ...
    # First from current level + first from level below
    new_val <- l_diffs[[curr_lvl]][1] - l_diffs[[curr_lvl+1]][1]
    # new_val <- l_diffs[[curr_lvl]][length(l_diffs[[curr_lvl]])] + l_diffs[[curr_lvl+1]][length(l_diffs[[curr_lvl+1]])]
  }
  # Prepend value to current level
  l_diffs[[curr_lvl]] <- c(new_val, l_diffs[[curr_lvl]])
  predictFirst(l_diffs, curr_lvl = curr_lvl-1)
}


l_diffs <- lapply(inputs, getDiffs)
l_preds <- lapply(l_diffs, predictFirst)
