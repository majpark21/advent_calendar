...
# Jokerize a hand - sets its jokers to maximize its type rank
jokerized_rank <- function(hand){
  sorted_hand <- sortString(hand)
  combis <- sapply(2:5, function(x){getNCombi(sorted_hand, x)})
  best_rank <- getType(combis[1], combis[2], combis[3], combis[4])
  
  order_cards <- c("J", as.character(2:9), "T", "Q", "K", "A")
  for(joker in order_cards){
    jokerized <- sortString(str_replace_all(hand, "J", joker))
    combis <- sapply(2:5, function(x){getNCombi(jokerized, x)})
    curr_rank <- getType(combis[1], combis[2], combis[3], combis[4])
...
isLarger <- function(hand1, hand2){
  order_cards <- 1:13
  names(order_cards) <- c("J", as.character(2:9), "T", "Q", "K", "A")
...
setorder(dt, jokerized_hand_type, rank_inType)
